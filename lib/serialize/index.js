...
function serializeVirtualPatchOrPatches(vPatch, functionIndex) {
  if (Array.isArray(vPatch)) {
    var len = vPatch.length;
    var res = new Array(len);
    var i = -1;
    while (++i < len) {
      res[i] = serializeVirtualPatch(vPatch[i], functionIndex);
    }
    return res;
  }
  return [serializeVirtualPatch(vPatch, functionIndex)];
}

function serializeVirtualPatch(vPatch, functionIndex) {
  var type = vPatch.type;
  var res = [
    type,
    vPatch.patch && vPatch.patch.a ? toJson(serializeRootPatch(vPatch.patch, functionIndex), functionIndex) : toJson(vPatch.patch, functionIndex)
  ];
...
function serializeRootPatch(patch, functionIndex) {
  var outputRootNode = serializeCurrentNode(patch.a, functionIndex);
  ...
  for (var key in patch) {
    if (key !== 'a') {
      res[key] = serializeVirtualPatchOrPatches(patch[key], functionIndex);
    }
  }
